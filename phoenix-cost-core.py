...
class CostReport:
    ...
    def config_overhead_tokens(self) -> int:
        return sum(c.est_tokens for c in self.config_overheads)
    ...
    if os.path.exists(SKILLS_DIR):
        ...
        for root, dirnames, filenames in os.walk(SKILLS_DIR):
            for f in filenames:
                if f.endswith(".md"):
                    try:
                        total_skills += os.path.getsize(os.path.join(root, f))
                        count += 1
                    except OSError:
                        pass
        overheads.append(ConfigOverhead(
...
def detect_waste(sessions: List[SessionTokens],
    ...
    # ── 4. Ghost skills ──
    for oh in overheads:
        if "Skill listing" in oh.description:
            total_skill_uses = sum(s.tool_calls.get("Skill", 0) for s in sessions)
            if total_skill_uses < 3 and oh.est_tokens > 50000:
                findings.append(WasteFinding(
                    id="ghost-skills",
                    category="ghost_skills",
                    severity="high",
                    description=(
                        f"Skills listing costs ~{oh.est_tokens:,} tokens/session. "
                        f"Only {total_skill_uses} skill invocations observed across "
                        f"{len(sessions)} sessions."
                    ),
                    estimated_token_waste=oh.est_tokens * len(sessions) // 2,
                    estimated_cost_waste=(
                        oh.est_tokens * len(sessions) / 2 / 1_000_000 * 0.50
                    ),
                    fix_command=(
                        "# Review ~/.claude/settings.json skills configuration. "
                        "Consider removing unused skill directories from auto-load."
                    ),
                    evidence=f"Skill directory: {oh.path}, {oh.line_count} skill files",
                ))
            break
...
def calculate_health(sessions: List[SessionTokens],
    ...
    # 5. Config overhead (15 pts)
    total_overhead = sum(o.est_tokens for o in overheads)
    if total_overhead < 50000:
        score += 15
    elif total_overhead < 100000:
        score += 10
    elif total_overhead < 200000:
        score += 5
