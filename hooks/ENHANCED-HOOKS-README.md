# PHOENIX Enhanced Hooks System v1.0

## 概述

PHOENIX Enhanced Hooks System 是对原有 hooks 系统的全面增强，提供了更完整的生命周期管理、智能触发条件、丰富的反馈机制和实时状态监控。
...
- **触发时机**: 记忆系统刷新时
- **功能**: 同步记忆到知识库、更新衰减分数
- **集成**: 使用现有的 `phoenix-auto-memory.py`
...
```bash
# 添加错误恢复通知
python3 ~/.claude/phoenix/hooks/notification-center.py add error_recovery high "工具执行失败，建议检查参数"

# 添加性能警告
python3 ~/.claude/phoenix/hooks/notification-center.py add performance_warning medium "上下文使用率超过 80%"
```
...
# 忽略通知
python3 ~/.claude/phoenix/hooks/notification-center.py dismiss <notification_id>
```
...
```bash
# 评估触发条件
python3 ~/.claude/phoenix/hooks/smart-trigger.py evaluate PostToolUse '{"tool_name":"Bash","error_count":3}'
...
# 导出状态数据
python3 ~/.claude/phoenix/hooks/status-indicator.py export --format json
```
...
# 手动测试 hook
echo '{"tool_name":"Bash","error_message":"test"}' | bash ~/.claude/phoenix/hooks/tool-error.sh

# 查看错误日志
cat ~/.claude/phoenix/tool-error-log.jsonl
```
...
# 检查静默时间
python3 -c "import json; print(json.load(open('$HOME/.claude/phoenix/notification-preferences.json'))['quiet_hours'])"
```
...
python3 -c "
import json
config = json.load(open('$HOME/.claude/phoenix/smart-trigger-config.json'))
config['global_settings']['enable_debug'] = True
json.dump(config, open('$HOME/.claude/phoenix/smart-trigger-config.json', 'w'), indent=2)
"
```
...
PHOENIX Enhanced Hooks System 是 PHOENIX 自进化 Agent 系统的一部分。
