#!/bin/bash
# PHOENIX NexSandglass — SessionStart 记忆注入
# 每次会话开始时，用 NexSandglass 蒸馏上下文替代 MEMORY.md 全量加载
# 目标: <300 tokens vs 原 8,000+ tokens

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PHOENIX_HOME="$HOME/.claude/phoenix"
CONTEXT_FILE="$PHOENIX_HOME/nexsandglass/session-context.md"

# Generate hybrid context
python3 "$PHOENIX_HOME/nexsandglass.py" hybrid > "$CONTEXT_FILE" 2>/dev/null || true
